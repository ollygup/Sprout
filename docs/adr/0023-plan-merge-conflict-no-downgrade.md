# Plan merges identical declarations, conflicts on differences, never downgrades

> Status: amended 2026-09-05 — original decision text preserved; see the executable-source audit amendment for current behavior and implementation gaps.

Composing presets is a pure function over their resolved Requirements, grouped by product. Same product with the same policy and step merges into one row (env and verify lists unioned, first declaration wins the rest) — the only silent case, safe because either declaration alone would do the same thing. Same product with a different policy or step is an explicit `conflict`: the row names its sources and candidates, offers no auto-pick, and blocks the run and the save-as-preset until the user picks one policy or excludes the row. A pinned requirement with a newer version installed is `SatisfiedByNewer` — reported, never downgraded to the pin. An installed-but-unmanaged product is an unmanaged skip, never force-adopted. A non-Windows preset file imports for reference with a warning and never plans cleanly. A Requirement whose product left the Library is `unresolved`: shown as "removed from library" with no candidates and excluded from detection and runs until the product is re-added — and it re-lights automatically when the id returns.

## Consequences

- The Plan screen's gating (`undecidedCount` blocks Start and Save) is a direct rendering of this semantics, not extra UI policy.
- Merge-then-conflict keeps the common case (two presets both wanting latest Git) to one row while keeping the dangerous case (pinned vs latest) always explicit.

## Amendment — 2026-09-05 (executable-source audit)

“Identical” means equal version policy and Step, not equivalent complete Requirements (`src-tauri/src/plan.rs`, `same_declaration` and `merge_requirement`). Env merging keys on `(action, name)` and retains the first value; timeout, dependencies, and Product metadata also remain those of the first declaration. Those differences can affect behavior, so either declaration does not necessarily have the same effect. Conflict-on-policy/Step-difference remains implemented; this correction does not approve silent resolution of other meaningful differences.

The planner and run loop implement the installed-newer skip (`plan_requirement` and the `SatisfiedByNewer` arm in `src-tauri/src/run.rs`). However, install/upgrade pass only Step, timeout, and directory to the adapter. Pinned policy never becomes `--version`, and the Step’s scope never becomes `--scope` (`src-tauri/src/engine/windows.rs`, `install`, `upgrade`, and `winget_args`). Pins influence action selection without enforcing the requested installed version. This is an implementation gap to address explicitly while preserving the no-downgrade decision.

A standalone unresolved reference has no candidates, but `compose` can combine an unresolved local reference with a resolved imported snapshot of the same id, producing an entry marked unresolved that nevertheless contains candidates. The unconditional no-candidates/excluded-from-runs description does not cover that mixed case. The UI still gates unresolved policy/Step conflicts; run-start Library freshness limitations are recorded in ADR-0007.
