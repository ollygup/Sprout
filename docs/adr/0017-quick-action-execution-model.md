...
> Status: amended 2026-09-06 — original decision text preserved; the authoring extension below is accepted but not implemented; the executable-source audit records current behavior and gaps.
...
## Amendment — 2026-09-06 (accepted AI authoring and shell extension; not yet implemented)

The user requested AI-assisted PowerShell and CMD authoring. A Quick Action will therefore carry an explicit shell, with legacy records interpreted as PowerShell. Run and stop commands use that action's selected shell, with invocation knowledge kept in the existing Windows execution owner. Ticket 147 covers storage, validation, manual editor/run/test/stop paths, and backup compatibility; it does not introduce a second runner.

Under ADR-0030, an AI Script draft becomes a Quick Action only through explicit user saving. AI generation and diagnosis cannot invoke the existing executable Test commands, run/stop actions, or execute a revision to verify it. User-operated execution controls remain separate. New generated actions have auto-run off, and AI cannot silently change the user's flags. Existing user-configured startup actions, current-process privileges, tracking, logging, and watchdog limitations remain unchanged. This is an accepted scope extension, not a claim that CMD or AI support is already shipped.
