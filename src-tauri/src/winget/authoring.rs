...
use super::{WingetProcess, NativeWinget, find_word};
...
fn run_winget(process: &impl WingetProcess, args: &[&str], timeout: Duration) -> Result<String, String> {
    let args: Vec<String> = args.iter().map(|s| s.to_string()).collect();
    let run = process.timed(&args, timeout);
    if run.timed_out {
        return Err(format!(
...
pub fn search(query: &str) -> Result<Vec<WingetMatch>, String> {
    search_with(&NativeWinget, query)
}

pub(super) fn search_with(process: &impl WingetProcess, query: &str) -> Result<Vec<WingetMatch>, String> {
    let out = run_winget(
        process,
        &[
            "search",
...
pub fn show(id: &str) -> Result<WingetShow, String> {
    show_with(&NativeWinget, id)
}

pub(super) fn show_with(process: &impl WingetProcess, id: &str) -> Result<WingetShow, String> {
    let out = run_winget(
        process,
        &[
            "show",
