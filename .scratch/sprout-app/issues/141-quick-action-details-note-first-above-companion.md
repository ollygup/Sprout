...
## Follow-up 6 — 2026-09-06 — system theme self-pinned after set_theme landed

- Symptom: with the OS on dark, picking System left the app light; light/dark pins worked, nothing reached the console.
- Cause: granting `allow-set-theme` activated the native sync, which pushed the *resolved* concrete value — but WebView2 reports the native window theme back through the same `prefers-color-scheme` surface the system resolution reads, so the forced value poisoned the next read and System could never leave it. Latent until the permission landed (denied calls fail silent, leaving the OS read honest).
- Fix: system mode passes null (follow system — the setTheme API's own shape) while the page keeps resolving its own tokens to the concrete OS value; pins still push concrete values. Pinned by `themeSystem.test.ts` (null-assertion failed red before the fix); the old `setTheme(applied)` source seam in `companionPane.test.ts` now pins the null contract.
