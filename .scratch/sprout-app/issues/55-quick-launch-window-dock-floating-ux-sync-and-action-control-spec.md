...
- **Auto-hide is Sprout-driven** *(superseded 2026-08-21 by ticket 63 — this bullet's original "stays OS-managed" premise was wrong: no OS mechanism moves an appbar; research `docs/research/0003-appbar-autohide-os-contract.md`)*, with the documented plumbing kept: `set_autohide` receives the *actual* docked edge; a real `uCallbackMessage` (from `RegisterWindowMessage`) is set before `ABM_NEW`; `ABN_STATECHANGE` keeps the frontend honest; engagement is verified via `ABM_GETAUTOHIDEBAR` and a failure is surfaced (transient blocked state, ticket 63), never swallowed — and hiding works even when refused. Floating windows never auto-hide. The ~16 ms cursor-polling driver animates the strip (180 ms ease-out) to a 2 px sliver and back as an **overlay**: auto-hide never reserves workspace space, so other windows keep their full size whether hidden or revealed (only `fixed` reserves).
...
- [ ] Auto-hide engages on the actual docked edge (verified via `ABM_GETAUTOHIDEBAR`); taskbar-style slide is Sprout-driven since ticket 63 (the OS never slides an appbar); failure surfaced
