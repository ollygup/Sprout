<script lang="ts">
  ...
  let {
    ...
    name: string;
    cardLabel: string;
    badge?: PacketBadge | null;
    index: number;
    animate?: boolean;
    expanded?: boolean;
    onmenu: (request: MenuRequest) => void;
    /** What activating the card does — root click and Enter/Space alike.
     *  Omit it and keyboard activation opens the card menu instead while
     *  clicks do nothing. */
    onactivate?: () => void;
    children: Snippet;
  } = $props();
  ...
  function menuFromCard(): MenuRequest {
    return { kind: "anchor", anchor: card ?? null, focusFirst: true, returnTo: card ?? null };
  }
...
>
  <div class="packet__band">
    <div class="packet__band-row">
      <h3 class="packet__name">{name}</h3>
      {#if badge}
        <span
          class="packet__pill"
          class:packet__pill--warm={badge.tone === "warm"}
          class:packet__pill--upper={badge.upper}
          title={badge.title}
        >
          {badge.text}
        </span>
